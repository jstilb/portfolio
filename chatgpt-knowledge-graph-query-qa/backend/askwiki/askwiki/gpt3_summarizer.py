...
class Gpt3Summarizer:
    ...
    def __init__(self, model=askwiki_davinci_fine_tune):
        config = configparser.ConfigParser()
        config.read('secrets.ini')
        openai.api_key = config['OPENAI']['OPENAI_API_KEY']
        self.endpoint_url = "https://query.wikidata.org/sparql"
        self.askwiki_davinci_fine_tune = model
    ...
    def sparql_to_df(self, endpoint_url, query):
    ...
    def run_prompt(self, prompt="", model="text-davinci-003", temperature=0.4, stop=None):
    ...
    def generate_response(self, input, stop=None):
        response = self.run_prompt(f"{input} ->", model=self.askwiki_davinci_fine_tune, stop=stop)
        # print(response)
    ...
    def run_sparql_and_summarize(self, sparql):
        wd_df, wikiobjects = self.sparql_to_df(self.endpoint_url, sparql)
        askwiki_openai_input, _ = self.getwikiprop_3(wd_df, wikiobjects)
        summary = self.generate_response(askwiki_openai_input)
        return wd_df, summary
