...
def getwikiprop(wikiobjects):
    ...
    for w in wikiobjects:
        ...
        # can we have pages where descriptions are not in english?
...
class T5Summarizer:
    def __init__(self):
        ...
        self.AskWiki_NLG = T5ForConditionalGeneration.from_pretrained('shrinivasbjoshi/W210T5NLGV2', return_dict=True)
        self.endpoint_url = "https://query.wikidata.org/sparql"

    def get_wiki_prop(self, items):
        result = getwikiprop(items)
        return result

    def generate_summary(self, wikibase_input):
        input_ids = self.tokenizer.encode(wikibase_input, return_tensors="pt")  # Batch size 1
        outputs = self.AskWiki_NLG.generate(input_ids)
